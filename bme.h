...
class BME {
	...
	void read_registers(byte reg, byte* res, int count) {
		...
		Wire.requestFrom(BME_ADDRESS, count);
		for (byte i = 0; i < count && Wire.available(); ++i) res[i] = Wire.read();
		return;
	}
	...
	void init(void) {
 	...
 	void sample(void) {
  		...
  		this -> read_registers(BME_DATA, (byte*) &this -> bme_data, sizeof(bme_data_t));
